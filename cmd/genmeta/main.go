...
// cleanOldGenFiles 删除上次 generate 产出的所有 cmd_gen_*.go 文件,
// 让本次 generate 从干净状态写入,避免遗留无效文件。
//
// 删除范围:
//   - cmd_gen.go (历史单文件,新版已不再产出但保留兼容)
//   - cmd_gen_<class>.go (本工具产出)
//   - cmd_gen_registry_test.go (本工具产出的测试 helper)
//
// 不删:
//   - cmd_generic.go (前缀冲突,实际是手写)
//   - cmd_gen_doc.go (手写 documentation)
//   - cmd_gen_meta_test.go / cmd_gen_full_test.go (手写测试)
func cleanOldGenFiles(root string) error {
	...
	for _, e := range entries {
		...
		if name == "cmd_gen_meta_test.go" || name == "cmd_gen_full_test.go" {
			continue
		}
		// 删除范围
		shouldDelete := false
		switch {
		case name == "cmd_gen.go":
			shouldDelete = true
		case name == "cmd_gen_registry_test.go":
			shouldDelete = true
		case strings.HasPrefix(name, "cmd_gen_") && strings.HasSuffix(name, ".go") &&
...
func generateFiles(spec *Spec) (map[string][]byte, error) {
	...
	// cmd_gen_registry_test.go: 测试用 384 项 (Name, Command) 注册表,
	// 与 spec.Commands 严格同步。文件名以 _test.go 结尾,只在测试编译期生效,
	// 不进入生产构建产物。
	{
		regBuf := &bytes.Buffer{}
		writeRegistry(regBuf, spec.Commands)
		formatted, err := format.Source(regBuf.Bytes())
		if err != nil {
			return nil, fmt.Errorf("format cmd_gen_registry_test.go: %w\n%s", err, regBuf.String())
		}
		out["cmd_gen_registry_test.go"] = formatted
	}

	return out, nil
}

// writeRegistry 输出 cmd_gen_registry_test.go 内容:
//
...
//	        ... (384 entries 按 Name 排序)
//	    }
//	}
//
// 文件名以 _test.go 结尾确保只在测试编译期参与,不进入生产二进制。
func writeRegistry(buf *bytes.Buffer, cmds []Meta) {
	fmt.Fprintln(buf, "// Code generated by cmd/genmeta. DO NOT EDIT.")
	fmt.Fprintln(buf, "// 与 specs/cmd_gen.yaml 严格同步; 新增/删除命令后由 'make cmdgen-generate' 自动维护。")
	fmt.Fprintln(buf, "package redisson")
	fmt.Fprintln(buf, "")
	fmt.Fprintln(buf, "// commandRegistryEntry 关联 yaml 中的 Name (Go 标识符) 与运行时 commandXxx 实例。")
	fmt.Fprintln(buf, "type commandRegistryEntry struct {")
	fmt.Fprintln(buf, "\tName    string  // yaml.name, 如 \"BitCount\"")
	fmt.Fprintln(buf, "\tCommand Command // 运行时实例")
	fmt.Fprintln(buf, "}")
	fmt.Fprintln(buf, "")
	fmt.Fprintf(buf, "// allRegisteredCommands 枚举本包所有 %d 个 CommandXxx 全局零值,\n", len(cmds))
	fmt.Fprintln(buf, "// 用 yaml.name 索引,避免 String() 在变体间共享导致歧义.")
	fmt.Fprintln(buf, "func allRegisteredCommands() []commandRegistryEntry {")
	fmt.Fprintln(buf, "\treturn []commandRegistryEntry{")

	// 按 Name 排序输出 (extractSpec 已排序,这里防御性再排一次)
	sortedNames := make([]string, 0, len(cmds))
	for _, m := range cmds {
		sortedNames = append(sortedNames, m.Name)
	}
	sort.Strings(sortedNames)
	for _, name := range sortedNames {
		fmt.Fprintf(buf, "\t\t{Name: %q, Command: Command%s},\n", name, name)
	}
	fmt.Fprintln(buf, "\t}")
	fmt.Fprintln(buf, "}")
}
...
func doCheck(root, specPath string) error {
	...
	// 比对 cmd_gen_registry_test.go: 应等同从 spec 重新生成的 registry 内容。
	// 新增/删除命令后若忘记 'make cmdgen-generate', 这里立即报错。
	if err := checkRegistryFile(root, specStored); err != nil {
		return err
	}
...
// checkRegistryFile 对比当前 cmd_gen_registry_test.go 与从 spec 重新生成的内容。
// 不一致时返回错误,提示运行 'make cmdgen-generate'。
func checkRegistryFile(root string, spec *Spec) error {
	regPath := filepath.Join(root, "cmd_gen_registry_test.go")
	current, err := os.ReadFile(regPath)
	if err != nil {
		return fmt.Errorf("read %s: %w", regPath, err)
	}
	buf := &bytes.Buffer{}
	writeRegistry(buf, spec.Commands)
	expected, err := format.Source(buf.Bytes())
	if err != nil {
		return fmt.Errorf("format expected registry: %w", err)
	}
	if !bytes.Equal(current, expected) {
		return fmt.Errorf("cmd_gen_registry_test.go drift from spec\n  fix: run 'make cmdgen-generate'")
	}
	return nil
}
