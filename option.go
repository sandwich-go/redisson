...
// 默认值用 const 而非 var：optiongen 解析 declaration 时拿到的若是裸 ident
// 会 panic（"got type ast.Ident, please give some type hint"），所以下面 declaration
// 中统一用 time.Duration(...) / int(...) 包一层，让 optiongen 拿到 *ast.CallExpr。
const (
